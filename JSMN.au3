...
Func _Jsmn_Shutdown()
	$_Jsmn_CodeBuffer = 0
	_API_MemVirtualFree($_Jsmn_CodeBufferPtr, 0, $MEM_RELEASE)
EndFunc
...
Func _Jsmn_Startup()
	If Not IsDllStruct($_Jsmn_CodeBuffer) Then
		...
		$_Jsmn_CodeBufferPtr = _API_MemVirtualAlloc(0, BinaryLen($Opcode), $MEM_COMMIT, $PAGE_EXECUTE_READWRITE)
...
Func Jsmn_Get($Var, $Key)
    ...
    Local $Match = StringRegExp($Key, "(?i)(^\[([^\]]+)\])", 3)
...
Func _API_MemVirtualAlloc($pAddress, $iSize, $iAllocation, $iProtect)
	Local $aResult = DllCall("kernel32.dll", "ptr", "VirtualAlloc", "ptr", $pAddress, "ulong_ptr", $iSize, "dword", $iAllocation, "dword", $iProtect)
	If @error Then Return SetError(@error, @extended, 0)
	Return $aResult[0]
EndFunc   ;==>_MemVirtualAlloc

Func _API_MemVirtualFree($pAddress, $iSize, $iFreeType)
